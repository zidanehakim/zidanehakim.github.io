...
export function Hero() {
    ...
    const tick = () => {
      ...
      if (!isDeleting) {
        ...
        if (charIdx > 0) {
          charIdx--;
          setText(word.slice(0, charIdx));
          timer = setTimeout(tick, 50); // Deleting speed (looks better when faster!)
        } else {
          isDeleting = false;
          wordIdx = (wordIdx + 1) % displayTexts.length;
          timer = setTimeout(tick, 500); // Brief pause before starting the next word
        }
      }
    };
  ...
  }, []); // Note: Ensure displayTexts is defined outside the component or wrapped in useMemo
  ...
  return (
    <section className="dot-grid relative w-screen h-screen bg-white overflow-hidden flex items-center justify-center">
      ...
      <div
        className="scroll-bob absolute bottom-6 hidden md:flex flex-col items-center gap-1.5 z-10"
        style={{ left: "50%" }}
      >
        <span className="text-[9px] font-mono text-gray-400 tracking-widest select-none">
          scroll
        </span>
